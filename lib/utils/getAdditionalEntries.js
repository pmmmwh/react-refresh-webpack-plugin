...
/** @typedef {{ prependEntries: import('./injectRefreshEntry').WebpackEntry, overlayEntries: import('./injectRefreshEntry').WebpackEntry }} AdditionalEntries */
 ...
 */
function getAdditionalEntries({ options, devServer }) {
  /** @type {Record<string, *>} */
  let resourceQuery = {};

  if (devServer) {
    const { sockHost, sockPath, sockPort, port, host } = devServer;
