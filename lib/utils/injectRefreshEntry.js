 ...
 */
function injectRefreshEntry(originalEntry, options, compiler) {
  /** @type {Record<string, *>} */
  let resourceQuery = {};

  if (compiler && compiler.options && compiler.options.devServer) {
    resourceQuery.domain = createDomain(compiler.options.devServer);
    if (compiler.options.devServer.sockHost)
      resourceQuery.sockHost = compiler.options.devServer.sockHost;
    if (compiler.options.devServer.sockPath)
      resourceQuery.sockPath = compiler.options.devServer.sockPath;
    if (compiler.options.devServer.sockPort)
      resourceQuery.sockPath = compiler.options.devServer.sockPort;
  }
