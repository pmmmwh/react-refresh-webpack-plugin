 ...
 * @param {WebpackEntry} prependEntries A Webpack entry that will be prepended
 * @param {WebpackEntry} overlayEntries A Webpack entry for the overlay
 * @returns {WebpackEntry} An injected entry object.
 */
function injectRefreshEntry(originalEntry, prependEntries, overlayEntries) {
      ...
      (acc, [curKey, curEntry]) => ({
        ...
        [curKey]:
          typeof curEntry === 'object' && curEntry.import
            ? {
                ...curEntry,
                import: injectRefreshEntry(curEntry.import, prependEntries, overlayEntries),
              }
            : injectRefreshEntry(curEntry, prependEntries, overlayEntries),
      }),
      {}
    );
  }
  ...
  if (typeof originalEntry === 'function') {
    return (...args) =>
      Promise.resolve(originalEntry(...args)).then((resolvedEntry) =>
        injectRefreshEntry(resolvedEntry, prependEntries, overlayEntries)
      );
  }
